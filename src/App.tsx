        ...
        {mode === 'child' && (
          <div className="bg-white p-6 rounded-2xl shadow-sm mb-6 text-center">
            ...
            <div className="relative w-32 h-32 mx-auto flex items-center justify-center">
              ...
              <div className="absolute text-3xl font-bold text-teal-600">
                {progress}%
              </div>
            </div>
            <p className="mt-2 text-sm text-gray-500">
              {doneCount} / {todaysTasks.length} タスク完了
            </p>
          </div>
        )}
        ...
        {tab === 'add' ? (
          ...
          <div className="space-y-3">
            ...
            {tasks.map((task) => (
              <div
                key={task.id}
                className={`bg-white p-4 rounded-xl shadow-sm border-l-4 flex items-center gap-3 transition-all
                ${
                  task.status === 'done'
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  {/* SOS表示 */}
                  {task.status === 'help' && (
                    <div className="text-red-500 font-bold text-xs flex items-center gap-1 mt-1 animate-pulse">
                      <AlertCircle size={14} /> 助けて！ここで止まっています
                    </div>
                  )}

                  {(task.duration > 0 || task.status === 'done') &&
                    task.status !== 'help' && (
                      <span
                    ...
                    <button
                      ...
                      className={`${
                        task.status === 'help'
                          ...
                          ? 'bg-orange-400'
                          : 'bg-teal-500'
                      } hover:opacity-90 text-white px-4 py-2 rounded-full font-bold shadow-md active:scale-95 transition-all flex items-center gap-1`}
                    >
  ...
  if (isFinishing) {
    return (
      <div className="fixed inset-0 bg-white/90 z-50 flex flex-col items-center justify-center animate-in fade-in zoom-in duration-300">
        ...
        <h2 className="text-3xl font-bold text-red-500 mt-4 animate-pulse">
          よくできました！
        </h2>
    ...
    <div className="bg-white p-6 rounded-2xl shadow-sm">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="pt-2 flex gap-3">
          ...
          <button
            type="submit"
            className="flex-1 bg-teal-500 text-white font-bold py-3 rounded-xl shadow-md hover:bg-teal-600 transition-colors"
          >
            追加する
          </button>
        </div>
  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-sm">
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          {categories.map((cat) => (
            <div
              key={cat}
              className="bg-teal-50 text-teal-700 px-3 py-1.5 rounded-lg text-sm font-bold flex items-center gap-2"
            >
              {cat}
              <button
      ...
      <form onSubmit={handleAdd} className="border-t pt-4">
        ...
        <div className="flex gap-2">
          <input
            className="flex-1 border bg-gray-50 rounded-lg p-3 outline-none focus:ring-2 focus:ring-teal-200"
            placeholder="例：ピアノ"
            value={newCat}
          ...
          <button
            type="submit"
            className="bg-teal-500 text-white px-4 rounded-lg font-bold hover:bg-teal-600"
          >
            追加
          </button>
        </div>
